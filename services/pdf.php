...
// SOMENTE AS FUNÇÕES QUE ESTÃO DENTRO DESTE EXTENDS É QUE SERÃO SUBSTITUIDAS.
    ...
    function Header(){ //CABECALHO
      ...
      //  $this->Cell(170,15,'PROTOCOLO DE JEJUM',0,0,'L'); 
        ...
        $this->SetXY(10,15);
        $this->Cell(0,$l,'LISTA DE NOMES - '.$_nome,'B',1,'C');
        $l=5;
        $this->SetFont('Arial','B',12);
...
//include("config.php"); // A MINHA CONEXÃO FICOU EM CONFIG.PHP
           ...
           include_once '../controller/class.atendimento_controller.php';
           include_once '../services/class.atendimentoListIterator.php';
           include_once '../model/class.atendimento.php';
           $ac = new atendimento_controller();
           
           $lista = $ac->listaAtendimento( $_id );
           $atdList = new atendimentoListIterator($lista);
            ...
            $item = 0;
            $itens = 0;
            while ($atdList->hasNextAtendimento()){
                $item++;
                $itens++;
                $atendimento = $atdList->getNextAtendimento();
            // ENQUANTO OS DADOS VÃO PASSANDO, O FPDF VAI INSERINDO OS DADOS NA PAGINA

                $dados1 = $item;
                $dados2 = utf8_decode($atendimento->getPaciente()->getNmPaciente()); // NESTE CASO, EU DECODIFIQUEI OS DADOS, POIS É UM CAMPO DE     TEXTO.
